...
GDG on Campus 대진대학교 운영진을 위한 동아리 관리 웹앱입니다. 회원 관리, 지원서 접수, 이벤트 등록, 신청/대기열, 출석 체크와 대시보드를 제공합니다.
...
3. 채널 ID(아래 방법으로 확인)를 `SLACK_ADMIN_CHANNEL_ID` 환경 변수에 등록합니다.

둘 중 하나라도 비어 있으면 발송하지 않고 이유를 알립니다. 꼬북봇 토큰으로 대신 나가지 않습니다.

#### 채널 ID 확인하는 법

채널 **이름**(`#운영진`)이 아니라 **ID**(`C`나 `G`로 시작하는 문자열)가 필요합니다. 셋 중 편한 방법을 쓰세요.

- **채널 세부정보** — 슬랙에서 채널을 열고 상단 채널 이름 클릭 → 팝업 맨 아래에 `채널 ID: C0123ABCDEF`와 복사 버튼이 있습니다.
- **링크 복사** — 채널 이름 우클릭 → "링크 복사" → 주소 끝부분이 ID입니다. `https://.../archives/C0123ABCDEF`
- **브라우저 주소창** — 웹 슬랙에서 채널을 열면 주소 마지막 조각이 ID입니다. `https://app.slack.com/client/T0000/C0123ABCDEF`

오래 전에 만든 프라이빗 채널은 `G`로 시작하기도 합니다. 둘 다 정상이니 보이는 그대로 넣으면 됩니다.
