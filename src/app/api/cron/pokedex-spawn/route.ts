...
export async function GET(request: NextRequest) {
  ...
  const [{ data: pokemon }, { data: config }] = await Promise.all([
    supabase.from("pokemon_catalog").select("name_ko").eq("id", due.pokemon_id).single(),
  ...
  const posted = await postMessage({ channel: config.channel_id, text: appearanceMessage(pokemon.name_ko, "pokeball"), botToken });
  if (!posted.ok) {
