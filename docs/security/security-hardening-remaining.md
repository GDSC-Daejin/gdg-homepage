...
> 원본 `docs/security/security-hardening-plan.md`의 P0(Task 1·2)는 **이미 구현·머지됨**(보안 헤더 + 공개 지원폼 남용 방지, 마이그레이션 `0035_application_abuse_guard.sql`). 이 문서는 **남은 Task 3~6만** 현재 코드 기준으로 다시 정리한 것이다.
