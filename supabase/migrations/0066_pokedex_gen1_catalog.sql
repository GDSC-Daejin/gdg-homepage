with gen1(pokedex_no, name_ko) as (
  select ordinality::int, name_ko
  from unnest(array[
    ...
    '슈륙챙이', '강챙이', '케이시', '윤겔라', '후딘', '알통몬', '근육몬', '괴력몬', '모다피', '우츠동',
  ...
  ]) with ordinality as catalog(name_ko, ordinality)
)
insert into pokemon_catalog (pokedex_no, name_ko, image_path, catch_rate, dwell_minutes, active)
select pokedex_no, name_ko,
  format('https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/%s.png', pokedex_no),
  0.5, 60, true
from gen1
on conflict (pokedex_no) do update
